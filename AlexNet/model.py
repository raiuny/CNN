...
class AlexNet(nn.Module):
    def __init__(self, num_classes = 1000, init_weight = False):
        ...
        self.features = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=3, stride=2), # output 48x27x27
            nn.Conv2d(48, 128, kernel_size=5, padding=2), # output 128x27x27
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2), # output 128x13x13
            nn.Conv2d(128, 192, kernel_size=3, padding=1), # output 128x13x13
            nn.ReLU(inplace=True),
